    ...
    if(window.WebSocket){
        ...
        websocketGame.socket.onmessage = function (e) {
            ...
            if (data.dataType === websocketGame.CHAT_MESSAGE) {
                ...
                // Überprüfen, ob websocketGame.clients existiert und ein Array ist
                if (Array.isArray(websocketGame.clients)) {
                    websocketGame.clients.forEach(function(client) {
                        console.log('Client.ID: ' + client.id);
                    });
                } else {
                    console.error("websocketGame.clients is not defined or not an array.");
                }
            }
...
// ****************************** clickListener ******************************
$("#joinUser").on("click", openMenu);

$("#openChat").on("click", joinChat);
$("#chatSend").on("click", sendMessage);
$("#newGame").on("click", newGame);
$("#joinGame").on("click", joinGame);
$("#mainSetup").on("click", setup);
$("#exitButton").on("click", exitButton);
$("#errorConfirmMain").on("click", errorMainWindowClose);
$("#failConfirmMain").on("click", errorMainWindowClose);

$("#chatroomBackButton").on("click", leftChat);

$("#startGame").on("click", confirmToGameReady);
$("#lobbyBackButton").on("click", lobbyBackButton);
$("#exitLobby").on("click", userExitLobby);
$("#deleteLobby").on("click", userDeleteLobby);
$("#errorConfirmDeleteLobby").on("click", errorLobbyWindowClose);
$("#confirmGameStopButton").on("click", gameStartWasCanceled);
$("#errorConfirmStatedLobby").on("click", errorMainWindowClose);

$("#setupBackButton").on("click", setupBackButton);
$("#changeUsername").on("click", setupNameChange);

$("#makeABetButton").on("click", showBetWindow);
$("#infoWindowButton").on("click", infoWindowButtonHandler);
$("#stopBetButton").on("click", closeInfoWindow);
$("#barrelGameRules").on("click", showGameRules);
$("#closeGameRulesButton").on("click", closeGameRules);
$("#hittingOrderButton").on("click", showTheHittingRank);
$("#closeHittingOrderButton").on("click", hiddeTheHittingRank);

$("#leaveButton").on("click", leaveButtonHandler);
$("#continueTheGameButton").on("click", continueTheGameButtonHandler);
...
function openInfoWindows(status){
    let fontSize = 20;
    let heightFactor;
