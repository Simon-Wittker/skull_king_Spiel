...
class Room{
    ...
    sendAll(message, senderId){
        for (let i = 0, length = this.users.length; i < length; i++){
            ...
            // console.log("*********** " + this.users[i].username + " ///// " + this.users[i].inChat + "*********")
            ...
            if(gameLobby.gameName === gameNameNew){
                for (let i = 0, j = 7; i < j ; i++){
                    if (gameLobby.playerInLobbyCounter === 6){
                        ...
                        // console.log("gameLobby.playerUserNames[gameLobby.playerInLobbyCounter] = " + gameLobby.playerUserNames[gameLobby.playerInLobbyCounter])
    ...
    getAllUsernamesInGameLobby(gameName, userId){
        ...
        this.gameLobbies.forEach(gameLobby => {
            if(gameLobby.gameName === gameNameNew){
                //console.log("gameLobby.gameName = " + gameLobby.gameName);
                for (let i = 0, j = gameLobby.playerUserNames.length; i < j ; i++){
                    //console.log("gameLobby.users[i].username = " + gameLobby.playerUserNames[i]);
                    usernameList += gameLobby.playerUserNames[i] + ":";
                    //console.log("getAllUsernamesInGameLobby(); gameLobby.playerUserId[i] =" + gameLobby.playerUserId[i])
        ...
        //requestingUser.socket.send(JSON.stringify(data));
    }
    ...
    sendConfirmGameStart(gameName){
        ...
        this.gameLobbies.forEach(gameLobby => {
            if(gameLobby.gameName === gameNameNew) {
                // console.log("gameLobby.playerInLobbyCounter = " + gameLobby.playerInLobbyCounter);
    ...
    setCardAndSendToAllPlayers(message, userId){
        ...
        // console.log("cardImg = " + JSON.stringify(cardImg));
        ...
        this.gameList.forEach(game => {
            if (game.gameId === gameId) {
                ...
                // console.log("game.playedCards = " + JSON.stringify(game.playedCards));
                // console.log("game.playerUserId = " + JSON.stringify(game.playerUserId));
        ...
        this.gameList.forEach(game => {
            if (game.gameId === gameId) {
                ...
                if (game.listOfContinueClickedPlayer.length === game.playerUserId.length){
                    // console.log(" +++++++++++++++++++++++++ handCardLength = " + handCardLength + " +++++++++++++++++++++++++ ");
...
class Game {
    ...
    findStich() {
        ...
        // No SkullKing
            ...
            console.log("this.playedCards[indexHighestNbr] = " + JSON.stringify(this.playedCards[indexHighestNbr]));
        ...
        for (let i = 0; i < this.playerUserId.length; i++) {
            if (this.playedCards[indexHighestNbr].owner === this.playerList[i].id){
                ...
                console.log("this.playerList[i].username = " + this.playerList[i].username);
            }
